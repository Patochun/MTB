...
def add_cube(collect, name_of_cube, material, x, y, z, empty_parent):
    """ Create a Cube Mesh
    IN
        collect         obj     collection
...
def add_fountain(collect, name_of_fountain, material, x, y, z, empty_parent):
...
def add_pb(collect, name_of_pb, material, x, y, z, empty_parent):
    """ Create a Paper Ball
    IN
        collect         obj     collection
...
def search_track_in_mtb_data(id):
    """ Search all infos about a track in mtb_data
    IN
        id         int     Identifier of track
    OUT
        mtb_track
    """    
    for p in mtb_data:
...
class Track_Class:
    ...
    def __init__(self, idx_track, track, name, visual_type):
        ...
        def Track_is_BG():
            ...
            pos_x = 0  #(((self.max_note - self.min_note) // 2) - 63) * self.loc_coef
            pos_y = self.idx * self.loc_coef
            empty_parent = add_empty(col_obj, empty_parent_name, pos_x, pos_y, 0)
            ...
            for x in range(self.min_note, self.max_note + 1):
                if x in self.list_note:
                    ...
                    add_cube(col_obj, cube_name, material, (x - 63) * self.loc_coef, 0, 0, empty_parent)
            return None
        ...
        def Track_is_LT():
            ...
            pos_x = 0  #(((self.max_note - self.min_note) // 2) - 63) * self.loc_coef
            pos_y = self.idx * self.loc_coef
            empty_parent = add_empty(col_obj, empty_parent_name, pos_x, pos_y, 4)
            # Create one light by used note
            for x in range(self.min_note, self.max_note + 1):
                if x in self.list_note:
                    light_name = col_name + "_" + str(x)
                    add_light(col_obj, light_name, (x - 63) * self.loc_coef, 0, 4, empty_parent)
            return None
        ...
        def Track_is_FT():
            ...
            for x in range(self.min_note, self.max_note + 1):
                if x in self.list_note:
                    ...
                    add_fountain(col_obj, fountain_name, material, (x - 63) * self.loc_coef, 0, 0, empty_parent)
            return None
        ...
        def Track_is_PB():
            ...
            for x in range(self.min_note, self.max_note + 1):
                if x in self.list_note:
                    ...
                    add_pb(col_obj, paperball_name, material, (x - 63) * self.loc_coef, self.idx * self.loc_coef, 0, empty_parent)
            return None

        """ ======= Main of __init__ ========================================== """

        # Know notes used in track and initialize her status
        if mid.type == 0:
        ...
        print('Generate Track {}: {}'.format(self.idx, self.name))
        ...
        # return True if exist some note in the track, else return False
        return None
    ...
    def add_evt(self, evt, frame, note, velocity):
        ...
        def FT_note_evt(frame, note, velocity):
            ...
            if velocity != 0:
                ...
                name_of_ps = note_name + "_PS_" + str(current_ps)
                ...
                ps.active.settings.object_align_factor[2] = 10
            ...
            # set frame_end of existing current PS
                ...
                ps_name = note_name + "_PS_" + str(current_ps)
...
# Else create it with BG type for all of tracks
mtb_data = [] # List of tracks
...
if (mid.type == 0):
    track = mid.tracks[0]
    l_channel = []
    # Fill l_channel with all channels found in track 0
    for msg in track:
        if msg.type == ('note_on'):
    ...
    for current_channel in l_channel:
        ...
        if jsoninit:
            ...
            mtb_track = {}
            mtb_track["Track"] = current_channel
            mtb_track["Name"] = track_name
            mtb_track["Type"] = "BG"
            mtb_data.append(mtb_track)
        else:
            mtb_track = search_track_in_mtb_data(current_channel)
            track_type = "BG"  # Default value
            if mtb_track != None:
                track_type = mtb_track["Type"]
            TrackList.append(Track_Class(current_channel, track, track_name, track_type))
    ...
    for current_track, track in enumerate(mid.tracks):
        ...
        if jsoninit:
            ...
            mtb_track = {}
            mtb_track["Track"] = current_track
            mtb_track["Name"] = track.name
            if len(TrackList[current_track].list_note) != 0:
                mtb_track["Type"] = "BG"
            else:
                mtb_track["Type"] = "XX"
            mtb_data.append(mtb_track)
        else:
            mtb_track = search_track_in_mtb_data(current_track)
            track_type = "BG"  # Default value
            if mtb_track != None:
                track_type = mtb_track["Type"]
            TrackList.append(Track_Class(current_track, track, track.name, track_type))
...
for current_track, track in enumerate(mid.tracks):
    print('Fill Track {}: {}, {} evt(s)'.format(current_track, track.name, len(track)))
